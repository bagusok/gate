...
export function LogArchivesClient() {
	...
	const summariesQuery = useQuery({
		queryKey: ["user-log-summaries", page, limit, startDate, endDate],
		queryFn: async () => {
			const params: Record<string, string | number> = { page, limit };
			if (startDate) params.startDate = startDate;
			if (endDate) params.endDate = endDate;

			const response = await api.api.user["log-summaries"].$get({ query: params as never });
	...
	const fetchSummaryDetail = async (summaryId: string) => {
		try {
			const response = await api.api.user["log-summaries"][":id"].$get({
				param: { id: summaryId },
			});
			const result = await response.json();
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">
						View archived daily summaries of your API requests
					</p>
				</div>
					...
					{summariesQuery.isLoading ? (
						...
						<>
							<div className="space-y-2">
								<div className="hidden md:grid grid-cols-6 gap-4 px-4 py-2 text-sm font-medium text-muted-foreground">
									<div>Date</div>
								...
								{summaries.map((summary: LogSummary) => (
									<button
										...
										className="grid grid-cols-2 md:grid-cols-6 gap-2 md:gap-4 rounded-lg border px-4 py-3 text-sm hover:bg-muted/50 cursor-pointer text-left bg-transparent"
									>
										<div className="flex items-center gap-2">
